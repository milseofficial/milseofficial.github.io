...
window.onscroll = function(ev) {
    if ((window.innerHeight + window.scrollY) == document.body.offsetHeight) retract();
    if(oldScrollY < window.scrollY){
		get('mena').style.opacity = "0";
		get('mena').style.transition = ".5s";
		setTimeout(menacloaker(1), 2000);
	} else {
		get('mena').style.opacity = "1";
		get('mena').style.transition = ".5s";
		setTimeout(menacloaker(0), 2000);
	}
	oldScrollY = window.scrollY;
};
function menacloaker(dependency){
	if (dependency == 1) {
		get('mena').style.display = "none";
	}else{
		get('mena').style.display = "block";
	}
}
